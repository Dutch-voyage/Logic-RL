...
def main(config):
    ...
    if False:
        ...
        template = "The reasoning process and answer are enclosed within <think> </think> and <answer> </answer> tags. Let's think step by step and output the final answer within <answer> </answer> tags."
        chat_lst = dataset[config.data.prompt_key].tolist()
        for chat in chat_lst:
            chat[0]['content'] = chat[0]['content'].split('Let\'s think step by step')[0] + template
            # Add an empty assistant message with <think> to start the response
            chat = np.concatenate([chat, np.array([{'role': 'assistant', 'content': '<think>'}, {'role': 'assistant', 'content': '<answer>'}])])
        ...
        def read_from_txt(path, filename):
            with open(os.path.join(path, filename), 'r') as f:
                return int(f.read())
        path = os.path.join(config.trainer.default_local_dir, 
                            f'global_step_{read_from_txt(config.trainer.default_local_dir, "latest_checkpointed_iteration.txt")}', 
                            "actor")
        wg.load_checkpoint(path)
