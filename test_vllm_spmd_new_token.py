...
def test_vllm_spmd():
    ...
    stop_token = "<early_stop>"
    num_added_tokens = tokenizer.add_special_tokens({"additional_special_tokens": [stop_token]})
    print(f"Added {num_added_tokens} token(s) to the tokenizer")
    
    stop_token_id = tokenizer.convert_tokens_to_ids(stop_token)
    print(f"Stop token ids: {stop_token_id}")

    preencode_prompts = [
        f"Who won the Champions League in 2019?",
    ]
    ...
    def early_stop_fn(token_ids, logprobs):
        return len(token_ids) >= 20
